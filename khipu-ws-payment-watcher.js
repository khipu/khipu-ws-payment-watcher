...
KhipuAtmosphere.statusServer = {
	...
	subscribe: function (options) {
		...
		statusRequest.onMessage = function (response) {
			...
			switch (message.lastEvent) {
				case 'automaton-downloaded':
					console.log('se descargo el automata')
					break;
				case 'payment-started':
					console.log('se inicio el pago')
					break;
				case 'notified-ok':
					console.log('el pago termino ok segun el banco origen')
					break;
				case 'conciliated':
					console.log('el pago fue conciliado')
					break;
			}
...
KhipuPaymentEvents = {
	initPaymentWatch: function (paymentId) {
		KhipuAtmosphere.statusServer.socket = atmosphere;
		KhipuAtmosphere.statusServer.subscribe({url: 'https://khipu.com/atm/payment/status/' + paymentId});
	},
};
